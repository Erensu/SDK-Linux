...
* \param pParam 用户定义的参数
*/
typedef void(CALL_METHOD *HMDIMUCallback)(double time, float accX, float accY, float accZ, float gyrX, float gyrY, float gyrZ, void* pParam);
/*
* \brief 设备IMU数据回调函数
* \param pParam 用户定义的参数
*/
typedef void(CALL_METHOD *HMDImageCallback)(double time, unsigned char* pLeft, unsigned char* pRight, int width, int height, int channel, void* pParam);
...
namespace indem {
    ...
    */
	...
	public:
		...
		* \param param 传给回调函数使用的参数
		*/
		void RegistHMDPoseCallback(DataCallback cb, void* param);
		/*
		* \brief 添加手柄位姿回调函数
		* \param cb 对应算法处理完毕后，用于数据传出的回调函数
		* \param param 传给回调函数使用的参数
        ...
        * \param param 传给回调函数使用的参数
        */
        void RegistHMDCameraCallback(HMDImageCallback cb, void* param);
        /*
        * \brief 注册头显原始IMU回调函数
        * \param cb 数据传出的回调函数
        * \param param 传给回调函数使用的参数
        */
        void RegistHMDIMUCallback(HMDIMUCallback cb,void* param);
